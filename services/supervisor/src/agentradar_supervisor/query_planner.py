...
async def generate_corroboration_queries(limit: int) -> list[str]:
    ...
    queries = [f"{item['concept']} agent framework OR protocol OR tool" for item in singletons]
    log.info(
        "query_planner.corroboration",
        count=len(queries),
        concepts=[s["concept"] for s in singletons],
    )
    return queries
...
async def generate_spike_queries(limit: int) -> list[str]:
    ...
    queries = [f"{item['concept']} announcement launch release" for item in spikes]
    log.info(
...
async def generate_adjacency_queries(limit: int) -> list[str]:
    ...
    queries = [
        f"new agent framework OR tool OR protocol from {item['authority']}" for item in authorities
    ]
    log.info(
