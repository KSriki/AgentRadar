...
class TestMcpProposerCritic:
    ...
    async def test_approve_commits_to_neo4j(
        ...
        assert any(
            edge["type"] == "INTRODUCED_BY" and edge["other"]["name"] == "Anthropic"
    ...
    async def test_reject_does_not_touch_neo4j(
        ...
        proposal = await mcp.call_tool(
            "propose_triple",
            {
                ...
                "predicate": "INTRODUCED_BY",
                "object": "NotReal",
                "source_id": "test:bogus",
                "confidence": 0.2,
            },
        )

        reject = await mcp.call_tool(
            "reject_triple",
            {
                "triple_id": proposal.data["triple_id"],
...
class TestMcpArtifacts:
    ...
    async def test_put_artifact_returns_uri(self, mcp, s3_client):
        ...
        # Verify retrievable
        body = await s3_client.get_artifact("test/integration_test_artifact.json")
